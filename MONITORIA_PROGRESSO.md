...
## Rodada de ajustes 2 (21/set/2026 — Correções.txt)
- **Usuários**: botão "Editar" agora fica na linha do título do modal (novo prop `acaoCabecalho` em `ModalPadrao`); "Operações vinculadas" virou select (Operador, 1 operação) ou dropdown com caixas (Supervisor/Qualidade/demais), ao lado do dropdown de supervisores; novos campos Turno, Equipe e **Canais de atendimento** (vínculo usuário↔canais, tabela `usuarios_canais`, só operações do próprio usuário — `mon_set_vinculos` valida). Componente reutilizável `SelectMultiplo` em `features/monitoria/comum.js`.
- **Central de Monitoria**: abas "Geral" (SLA, identidade, zona de risco) e "Guia de processos" (tópicos dobráveis, "Expandir/Recolher tudo", conteúdo com altura ajustável). Espaçamentos em grade de 8px, botões curtos.
- **Logs**: "Logs da Monitoria" virou sub-aba de Configurações > Logs (só Administrador); o link saiu do submenu. A rota `screen-settings-monitoria-logs` continua abrindo a sub-aba (links antigos).
- **Formulários**: `.mon-matriz` com `width: 47rem` (max 100%); linha de bloco dobrável (recolhe as perguntas e mostra a contagem).
- **Contestação (fluxo já existia; só apresentação + notificações)**: operador vê "Concordar" (ícone) e "Contestar feedback"; etiquetas por perfil — operador "Em contestação", demais "Contestada", ao dar baixa "Contestação encerrada" (`etiquetaContestacao` em `comum.js`; a lista devolve `tem_contestacao`). "Dar baixa" = concluir a reanálise (manter/anular) → FINALIZADA/histórico, sem edição. Supervisor ganhou "Minhas monitorias" (monitorias da equipe; regra só no frontend: `podeAcessarTela` + menu). Notificações: supervisor recebe "monitoria X foi contestada por Y"; operador recebe "Contestação encerrada".
- **Operações**: formulário em blocos dobráveis (Identificação também) e espaçamento revisado; nova aba **Configurar ambiente** (`GET/PUT /monitoria/operacoes/{chave}/ambiente`, permissões `configuracoes.visualizar/editar`): supervisores, Qualidade (flag `possui_qualidade` em `operacoes_ambiente`), turnos e equipes e **intranets** (ambientes SharePoint: `ambientes_sharepoint.operacao_id`, uma operação por intranet; só atribuição, o Mural não foi restringido). Não remove supervisor que ainda supervisiona operadores (usar "Transferir supervisão").
- **Logs / armazenamento**: `services/log_archiver.py` + job diário 03:00 (APScheduler): exporta `logs_auditoria` mais antigos que `RH_LOG_ARCHIVE_DAYS` (padrão 90, mínimo 30) para ZIP em `RH_LOG_ARCHIVE_DIR` e só então remove do banco; `RH_LOG_ARCHIVE_ENABLED=false` desliga. `monitoria_logs` (imutável) fica de fora.
- Migration **V039** (tabelas `usuarios_canais` e `operacoes_ambiente`), gerada de `monitoria_schema.py` (teste de igualdade em `test_monitoria_ambiente.py`); já aplicada no banco DEV.
